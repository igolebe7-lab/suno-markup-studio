...
function TagLibrary({ onConfigure }: { onConfigure: (tag: Tag) => void }) {
  ...
  return (
    <aside className="tag-library" data-testid="tag-library">
      <div className="panel-title">Библиотека тегов</div>
...
function StylePromptEditor() {
  ...
  return (
    <section
      className={`editor-panel style-panel ${isOver ? 'over' : ''}`}
    ...
    >
      <div className="section-header">
        <div>
          <h2>Стиль / жанр</h2>
          <p>Chips, raw prompt и drop zone для Suno Style field</p>
        </div>
        <div className="mode-tabs">
          <button className="active">chips</button>
          <button>raw</button>
      ...
      <div className="chip-row">
        {chips.map((tag) => (
          <button className="chip" key={tag.id} onClick={() => removeStyleTag(tag.id)}>
            {tag.sunoText}<X size={13} />
          </button>
        ))}
        <span className="drop-hint">Перетащите style-тег сюда</span>
      </div>
      <textarea
  ...
  return (
    ...
    >
      <div className="section-header">
        <div>
          <h2>Текст песни</h2>
          <p>Plain text, line numbers, подсветка метатегов, autocomplete по “[”</p>
        </div>
...
function RightPanel() {
  ...
  return (
    <aside className="right-panel">
      ...
      <section className="side-block" id="export-panel">
